...
class SDPA(NNFactory):
    ...
    def run(
        ...
        if query.ndim != 4 or key.ndim != 4 or value.ndim != 4 or mask.ndim != 4:
            logger.error("Invalid tensor dimensions: query=%s, key=%s, value=%s, mask=%s", query.shape, key.shape, value.shape, mask.shape)
            raise ValueError("All input tensors must be 4-dimensional")
        if query.shape != self.query.shape:
            logger.error("Query shape mismatch: got %s, expected %s", query.shape, self.query.shape)
        ...
        if key.shape != self.key.shape:
            logger.error("Key shape mismatch: got %s, expected %s", key.shape, self.key.shape)
            raise ValueError(f"Key tensor shape {key.shape} doesn't match expected shape {self.key.shape}")
        if value.shape != self.value.shape:
            logger.error("Value shape mismatch: got %s, expected %s", value.shape, self.value.shape)
        ...
        if mask.shape != self.mask.shape:
            logger.error("Mask shape mismatch: got %s, expected %s", mask.shape, self.mask.shape)
