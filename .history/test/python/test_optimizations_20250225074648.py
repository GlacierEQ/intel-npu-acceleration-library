...
from intel_npu_acceleration_library.optimizations import optimize_function
from intel_npu_acceleration_library.dtypes import bfloat16, float32


def test_optimize_function_with_bfloat16():
    """Test optimization function with BFloat16."""
    input_data = [bfloat16(1.0), bfloat16(2.0)]
    optimized_data = optimize_function(input_data)
    assert optimized_data is not None
    assert all(isinstance(x, bfloat16) for x in optimized_data)
