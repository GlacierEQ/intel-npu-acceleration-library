...
from intel_npu_acceleration_library.dtypes import bfloat16


def test_quantization_with_bfloat16():
    """Test quantization of a tensor to BFloat16."""
    input_tensor = torch.tensor([1.0, 2.0, 3.0], dtype=torch.float32)
    quantized_tensor = quantize_tensor(input_tensor, target_dtype=bfloat16)

    assert quantized_tensor.dtype == bfloat16.torch_dtype
    assert quantized_tensor.shape == input_tensor.shape
    assert torch.allclose(quantized_tensor.float(), input_tensor, rtol=1e-2, atol=1e-2)
